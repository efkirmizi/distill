...
class ShuffleNet(nn.Module):
    ...
    def forward(self, x, is_feat=False, preact=False):
        out = self.bn1(self.conv1(x)) #F.relu(self.bn1(self.conv1(x)))
        f0 = out
        out, f1_pre = self.layer1(out)

        f1_act = [self.relu(f) for f in f1_pre]

        #f1 = out
        out, f2_pre = self.layer2(out)

        f2_act = [self.relu(f) for f in f2_pre]

        #f2 = out
        out, f3_pre = self.layer3(out)

        f3_act = [self.relu(f) for f in f3_pre]
        ...
        if is_feat:
            if preact:
                ...
                return [self.relu(f0)] + f1_act + f2_act + f3_act + [f4], out
        else:
            return out
